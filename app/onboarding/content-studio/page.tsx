...
function contextualPreviewSlides(template: DisplayStyle, angle: string) {
  const headline = angle && angle !== "交由 AI 決定" ? angle : "由一個重點開始說清故事";
  if (template.code.includes("product")) return [
    { label: "封面", title: headline },
    { label: "P.2", title: "先看產品本身" },
    { label: "P.3", title: "再看製作與細節" },
  ];
  if (template.code.includes("problem")) return [
    { label: "封面", title: headline },
    { label: "P.2", title: "常見做法有何不足？" },
    { label: "P.3", title: "真正分別在哪裡？" },
  ];
  if (template.code.includes("bold")) return [
    { label: "封面", title: headline },
    { label: "P.2", title: "先看最關鍵一幕" },
    { label: "P.3", title: "三點快速說明" },
  ];
  return [
    { label: "封面", title: headline },
    { label: "P.2", title: "從原材料開始" },
    { label: "P.3", title: "一步一步看製作過程" },
  ];
}
                        ...
                        <button key={template.code} type="button" className={selectedStyleCode === template.code ? "active" : ""} onClick={() => setSelectedStyleCode(template.code)}>
                          <div className="contextual-preview" data-style={template.code}>
                            {slides.map((slide, slideIndex) => (
                              <div key={slide.label} className={slideIndex === 0 ? "cover" : ""} style={{ background: template.palette[0], color: template.palette[1] }}>
                          ...
                          <div className="template-copy">
                            <span>{index === 0 ? "SOON 建議" : "可選風格"}</span>
                            ...
                            <b>{selectedStyleCode === template.code ? "✓ 已選擇" : "選用這款"}</b>
                          </div>
                        </button>
                      )})}
                    </div>
...
const styles = `
  ...
  .contextual-preview{display:grid;grid-template-columns:1.55fr 1fr;grid-template-rows:1fr 1fr;gap:5px;height:205px;padding:8px;background:#eee8e2}.contextual-preview>div{position:relative;min-width:0;overflow:hidden;border-radius:7px;padding:11px;display:flex;flex-direction:column;justify-content:flex-end;align-items:flex-start;gap:5px;text-align:left}.contextual-preview>div.cover{grid-row:1/3}.contextual-preview i{position:absolute;right:-12px;top:-12px;width:48px;height:48px;border-radius:50%;opacity:.95}.contextual-preview small{position:relative;font-size:7px;font-weight:850;letter-spacing:.08em;text-transform:uppercase;opacity:.7}.contextual-preview strong{position:relative;font-size:11px;line-height:1.15;letter-spacing:-.025em}.contextual-preview .cover strong{font-size:18px}.contextual-preview[data-style*="product"]>div{justify-content:center}.contextual-preview[data-style*="problem"] .cover strong{font-size:16px}.contextual-preview[data-style*="bold"] strong{text-transform:uppercase;font-weight:900}.template-copy em{color:#777b83;font-size:9px;font-style:normal}.template-copy>b{margin-top:3px;border-radius:8px;background:#f1ebe4;color:#6b2c30;padding:8px;text-align:center;font-size:10px}.style-template-grid>button.active .template-copy>b{background:#6b2c30;color:#fff}
