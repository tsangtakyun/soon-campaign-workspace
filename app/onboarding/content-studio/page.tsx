                        ...
                        <button key={template.code} type="button" className={selectedStyleCode === template.code ? "active" : ""} onClick={() => setSelectedStyleCode(template.code)}>
                          {isClearMagazineCarousel(template.code) ? (
                            <div className="actual-style-preview">
                              <NextImage
                                src={clearMagazineCarouselV1.previewPath}
                                alt="清晰雜誌風的封面、內容頁及結尾頁實際預覽"
                                fill
                            ...
                            {slides.map((slide, slideIndex) => (
                              <div key={`${slide.role}-${slideIndex}`} className={slide.role} data-layout={slide.layout || "standard"} style={{ background: template.palette[0], color: template.palette[1] }}>
                                ...
                                <i className="preview-image-area" style={{ background: template.palette[2] }} aria-hidden="true" />
